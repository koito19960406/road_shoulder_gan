...
class Visualizer:
    ...
    def cyclegan_generate_stats_from_log(self, model, experiment_name, line_interval=10, nb_data=10800, enforce_last_line=True):
        """
        ...
        if os.path.exists(f"./reports/figures/{model}_{experiment_name}.png"):
            print(f"./reports/figures/{model}_{experiment_name}.png" + " already exists")
        else:
            #extract every lines
            with open(os.path.join(self.root_dir, "models", experiment_name, model, "loss_log.txt"), 'r') as f:
                lines = f.readlines()
            ...
            plt.legend(loc="best")
            plt.savefig(f"./reports/figures/{model}_{experiment_name}.png")
            # plt.show()
            # plt.close()
            
    def pix2pix_generate_stats_from_log(self, model, experiment_name, line_interval=10, nb_data=10800, enforce_last_line=True):
        """
        Generate chart with all losses from log file generated by CycleGAN/Pix2pix/CUT framework
        """
        # run if the output doesn't exist yet
...
if __name__ == '__main__':
    ...
    for experiment_name in os.listdir(os.path.join(root_dir,"models")):
        if not experiment_name.startswith('.'):
            for model in os.listdir(os.path.join(root_dir,"models", experiment_name)):    
                visualizer = Visualizer(root_dir)
                if model == "bicycle_cyclegan":
                    visualizer.cyclegan_generate_stats_from_log(model, experiment_name, line_interval=10, nb_data=3473)
                if model == "bicycle_pix2pix":
                    visualizer.pix2pix_generate_stats_from_log(model, experiment_name, line_interval=10, nb_data=3473)