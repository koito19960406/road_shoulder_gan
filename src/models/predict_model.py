...
class CustomPredictor:
    def __init__(self, data_root, check_point, results_dir, model_name_with_gan):
        self.data_root = data_root
        self.check_point = check_point
        self.results_dir = results_dir
    ...
    def _get_with_gan(self, model_name_with_gan):
        if "with_gan" in model_name_with_gan:
            return "with_gan"
    ...
    def load_model(self):
        if self.model_name == "lightgbm":
            ...
            # load from torch
            model = torch.load(self.check_point)
    ...
    def _predict_lightgbm(self, target):
        ...
        # load data
        df = pd.read_csv(os.path.join(self.data_root, "test.csv"))
        if self.with_gan == "with_gan":
            predictors = [col for col in df.columns if ('_gan' in col) or ('_gsv' in col)]
        else:
            predictors = [col for col in df.columns if '_gsv' in col]
        # run prediction
        df["predicted_" + target] = model.predict(df[predictors])
        # save to csv
        df.to_csv(os.path.join(self.results_dir, f"{target}_{self.model_name}_predictions.csv"), index=False)
    ...
    def _predict_resnet(self, target):
        ...
        # run prediction
        dataset = ImageDataset(df[['filename_key', target] + predictors], self.data_root, transform=self.transform, with_gan=self.with_gan)
